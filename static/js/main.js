(function ($) {
  ...
  */
  ...
  $("#phishingForm").submit(function (event) {
    ...
    // Get the input value
    const inputUrl = $("#inputUrl").val().trim();

    // Simulate phishing detection (replace with actual API call)
    const isPhishing = detectPhishing(inputUrl);

    // Display result
    const resultText = isPhishing
      ? '<span class="text-danger">⚠️ Phishing detected!</span>'
      : '<span class="text-success">✅ No phishing detected.</span>';
    $("#result").html(resultText);
  });

  // Simulated phishing detection function
  function detectPhishing(input) {
    // Replace this with actual phishing detection logic or API call
    const phishingKeywords = ["login", "password", "bank", "paypal", "verify"];
    return phishingKeywords.some((keyword) => input.toLowerCase().includes(keyword));
  }
  });
  
  $(document).on("mousemove", function(event) {
            let fish = $("#fish");
            let x = event.clientX + 120; // Offset distance from mouse
            let y = event.clientY; 
            fish.css({
                left: x - fish.width() / 2,
                top: y - fish.height() / 2,
